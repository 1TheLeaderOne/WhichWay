...
class PlayerExt extends lib.element.Player {
  /**
   * 移除手牌中所有的（驶舰之向）提示
  ...
  async reUnseen(noChange) {
    ...
    if (noChange != false) {
      ...
      player.when("showCharacterAfter").then(() => {
        player.hp = hp;
        player.maxHp = maxHp;
  ...
  canUseCardAtt(card, isfriend, distance) {
    if (distance === void 0) distance = true;
    if (isfriend === void 0) isfriend = true;
