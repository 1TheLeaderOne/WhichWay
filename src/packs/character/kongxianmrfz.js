import { get, _status, ui } from "noname";
...
skill({
  ...
  "sanyimrfz": {
    ...
    async content(event, trigger, player) {
      ...
      }).set("prompt", `【散逸】:你可以额外指定任意名体力值之和不超过${trigger.card.number - trigger.targets[0].hp}的角色`).set("selectTarget", [0, Infinity]).set("complexTarget", true).set("ai", (target) => {
        return get.effect(target, _status.event.cardx, _status.event.player, _status.event.player) > 0;
      }).set("targets", trigger.targets).set("targetx", trigger.targets[0]).set("cardx", trigger.card).forResult();
  ...
  "baofengmrfz": {
    ...
    async content(event, trigger, player) {
      const { bool } = await player.chooseUseTarget(
        ...
        false,
        false
      ).set("prompt", `【追矢】:你可以视为使用一张无色且点数为${trigger.card.number}的${get.translation(trigger.card.nature)}【杀】`).forResult();
      if (!bool) return;
    }
  }
});
...
//# sourceMappingURL=kongxianmrfz.js.map
