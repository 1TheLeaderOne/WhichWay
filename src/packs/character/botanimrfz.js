...
skill({
  lingshengmrfz: {
    ...
    async content(event, trigger, player) {
      ...
      const { links } = await player.chooseCardButton({
        prompt: "请选择获得一张牌",
        cards,
        forced: true,
        ai(button) {
          const { player: player2, targetx, answer: answer2 } = event;
          if (player2.hasSkillTag("viewHandcard", null, targetx, true) && answer2 !== void 0) {
            return button.link === answer2 ? 1 : 0;
          }
          return get.value(button.link, player2);
        }
      }).set("answer", answer).set("targetx", source).forResult();
      if (!links) return;
      const card = links[0];
      player.gain({
        cards: [card],
        source: card === answer ? source : void 0,
        animate: "giveAuto"
      });
      if (card !== answer) {
        ...
        player.when({ global: "phaseEnd" }).step(async (event2, trigger2, player2) => {
          player2.enableSkill("lingshengmrfz");
        }).translation("聆声").assign({
          mark: true,
          intro: {
  ...
  yingshengmrfz: {
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player.chooseControl({
        ...
        ai(event2, player2) {
          ...
          if (get.attitude(player2, target) > 0 && player2.canUse("guaguliaodu", target)) {
            return "刮骨疗毒";
          }
          if (get.effect(target, get.autoViewAs({ name: "juedou" }), player2, player2) > 0 && player2.canUse("juedou", target)) {
            return "决斗";
          }
          return "cancel2";
        }
      }).set("targetx", source).set("canUseTao", canUseTao).forResult();
      event.result = {
        ...result,
        cost_data: result
      };
    ...
    subSkill: {
      check: {
        ...
        async content(event, trigger, player) {
          const data = trigger.card.storage.yingshengmrfz;
...
function canUseTao(player, target) {
  if ([target, player].includes(void 0)) {
    return false;
  }
