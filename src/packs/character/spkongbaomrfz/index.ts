	...
	xuzhanmrfz: {
		...
		async content(event, trigger, player) {
			...
			if (trigger.targets.length < 1) {
				await player.draw();
				if (!player.hasUseTarget(trigger.card, undefined, false) || player.countCards("he") < 1) {
					return;
				}
				const result = await player
					.chooseCardTarget({
						...
						filterCard: true,
						filterTarget(card, player, target) {
							return player.canUse(get.event().cardname, target, undefined, false);
						},
						ai2(target) {
							const { player, cardname } = get.event();
							return get.effect(target, cardname, player, player);
						},
					})
						...
						.step(async (event, trigger, player) => {
							await player
								.chooseUseTarget({
									addCount: false,
									forced: true,
									card: get.autoViewAs({ name: get.name(trigger.card) }),
									cards: result.cards,
									filterTarget(card, player, target) {
...
function getShaCount(player: Player) {
