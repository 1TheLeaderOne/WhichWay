...
import { ui, get, game, _status, lib } from "noname";
...
translate({
  ...
  piaolingmrfz: "飘零",
  piaolingmrfz_info: "任意角色回合结束后，若你本回合使用过有点数的牌，你可以展示弃牌堆顶的X张牌，任意角色可选择展示其手牌中与展示的牌同名的牌，然后你与其摸一张牌并将展示牌中与此同名的牌插入牌堆中。（X=你本回合最后一张使用的有点数的牌的点数）"
});
skill({
  fengxumrfz_youyimrfz: {
    ...
    filter(event, player) {
      const cards = player.storage.fengxumrfz_youyimrfz;
    ...
    async content(event, trigger, player) {
      ...
      if (game.hasPlayer((char) => char.getDamagedHp() > 0 && char !== player)) {
        const { targets: [target] = [] } = await player.chooseTarget().set("prompt", `【游医】:你可以令一名其他角色回复一点体力、模一张牌且使用一张牌`).set("filterTarget", (card, player2, target2) => player2 !== target2 && target2.getDamagedHp() > 0).set("ai", (target2) => {
          const player2 = get.player();
          if (get.attitude(player2, target2) < 0) return -1;
          return target2.getDamagedHp() + get.attitude(player2, target2);
        }).forResult();
        if (target && target.isAlive()) target.recover();
      }
      await player.chooseToUse().set("prompt", `【游医】:你可以使用一张手牌`);
    },
  ...
  piaolingmrfz: {
    ...
    filter(event, player) {
      return getNumber(player) !== void 0 && ui.discardPile.children.length > 0;
    },
    prompt2(event, player) {
      return whichWayUtil.colorize(`你可以展示弃牌堆顶的#r${getNumber(player)}#张牌，任意角色可选择展示其手牌中与展示的牌同名的牌，然后你与其摸一张牌并将展示牌中与此同名的牌插入牌堆中。`);
    },
    async content(event, trigger, player) {
      const num = getNumber(player);
      const cards = [];
      for (let i = 0; i < num; i++) {
        if (ui.discardPile.children.length > 0) {
          let card = get.discardPile(true, "top");
          cards.push(card);
          await game.cardsGotoOrdering([card]);
        }
      }
      if (cards.length < 1) return;
      player.showCards(cards).set("prompt", `【飘零】:展示了${cards.length}张牌`);
      let names = Array.from(new Set(cards.map((card) => card.name)));
      for (const char of game.players.sortBySeat(_status.currentPhase)) {
        const { cards: [card] = [] } = await char.chooseCard().set("prompt", `【飘零】:你可以展示一张牌，${names.length > 1 ? `若牌名为${get.translation(names)}中的一个，${char === player ? "你" : get.translation(player)}可执行一些操作` : "然后无事发生"}`).set("ai", (card2) => {
          const player2 = get.player(), target = get.event().target;
          const names2 = get.event().names;
          if (get.attitude(player2, target) < 0) return -1;
          if (names2.includes(card2.name)) return 1;
          return -1;
        }).set("target", player).set("names", names).forResult();
        if (!card) continue;
        if (!names.includes(card.name)) continue;
        await char.showCards([card]).set("prompt", `${get.translation(char)}展示了一张牌`);
        ...
        await game.cardsGotoPile(removes).set("insert_index", () => {
          return ui.cardPile.childNodes[get.rand(0, ui.cardPile.childNodes.length - 1)];
        });
        game.log(removes, `被随机插入到牌堆之中`);
        names = Array.from(new Set(cards.map((card2) => card2.name)));
      }
...
function getNumber(player) {
  if (player.getHistory("useCard", (evt2) => typeof evt2.card.number === "number").length < 1) return void 0;
