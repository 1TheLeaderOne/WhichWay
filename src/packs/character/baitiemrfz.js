...
skill({
  "jigongmrfz": {
    ...
    async content(event, trigger, player) {
      let result;
      const next = player.chooseControl().set("choiceList", [
    ...
    subSkill: {
      gcard: {
        ...
        async content(event, trigger, player) {
          let result;
          result = await player.chooseTarget("你可以将【支援装备】移动至一名其他角色的装备区", (card, player2, target) => {
            return target !== player2 && !target.getEquip(5) && !target.isDisabled(5);
          }).set("ai", (target) => {
        ...
        async content(event, trigger, player) {
          ...
          for (var i = 0; i < 3; i++) {
            if (trigger.player.countCards("e", list[i])) {
              let card = trigger.player.getCards("e", function(card2) {
  ...
  "jiefeimrfz": {
    ...
    async content(event, trigger, player) {
      ...
      if (result.color === "red") {
        await player.draw();
        result = await player.chooseTarget("你可以令一名角色装备【支援装备】", (card, player2, target) => {
...
//# sourceMappingURL=baitiemrfz.js.map
