			...
			event.result = await player
				.chooseTarget({
					...
					filterTarget(card, player, target) {
						const targets = get.event().targetsx as Player[];
			...
			const result = await player
				.choosePlayerCard({
					...
					ai(button) {
						...
						if (link.isKnownBy(player)) {
							let num = 0;
							const card = get.event().cardx as Card;
				...
				if (acts.includes("extraDamage")) {
					player
						...
						.filter((event, player) => {
							...
							return event.card && event.card === trigger.card;
						})
...
function getLastUsed(player: Player, event: GameEvent): GameEvent | false {
