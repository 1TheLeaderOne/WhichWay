...
skill({
  "chongxiemrfz": {
    ...
    async content(event, trigger, player) {
      const { targets } = await player.chooseTarget(get.prompt("chongxiemrfz"), "你可以弃置其中一名角色的手牌", function(card, player2, target2) {
        return (_status.event.targets.includes(target2) || target2 == player2) && target2.countDiscardableCards(player2, "h");
  ...
  "leitingmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      const { targets } = await player.chooseTarget(true, "【雷霆】:请选择一个其他角色，对其使用6张雷【杀】").set("filterTarget", (card, player2, target) => {
        return player2.canUse({ name: "sha", nature: "thunder", isCard: true }, target, false) && player2 != target;
      }).set("ai", (target) => {
...
//# sourceMappingURL=qiaojiakelifumrfz.js.map
