...
// lib.commonArea.set(ORB,{
//     translate:"充能球区",
//     areaStatusName:ORB,
//     fromName:`from${ORB}`,
//     toName:`to${ORB}`,
//     addHandeler(event:GameEvent, trigger:GameEvent, player:Player) {
//         const { cards } = event;
//         (_status[ORB] as Array<any>).addArray(cards.filter(card=>{
//             return !card.willBeDestroyed(ORB,null,event.relatedEvent);
//         }));
//         if((_status[ORB] as Array<any>).length>3){
//             const cards2 = (_status[ORB] as Array<any>).splice(0,(_status[ORB] as Array<any>).length - 3);
...
skill({
	jibaomrfz: {
		...
		intro: {
			name: "充能球区",
			mark(dialog, storage: Array<Card>, player) {
				if (storage.length < 1) {
					dialog.addText("无");
                    return;
				}
		...
		onremove(player, skill) {
			const cards = player.storage[skill];
		...
		async content(event, trigger, player) {
            ...
            (player.storage.jibaomrfz as Card[]).addArray(cards);

            const orbs = player.storage.jibaomrfz as Card[];
            if(orbs.length>3){
                const cards2 = orbs.slice(0,orbs.length - 3);
                for(let card of cards2){
                    (player.storage.jibaomrfz as Card[]).remove(card);
        ...
        subSkill:{
            round:{
                ...
                filter(event, player, name, target) {
                    return (player.storage.jibaomrfz as Card[]).length>0;
                },
                async content(event,trigger,player){
...
async function orb(card:Card,player:Player,type: "jifa" | "beidong"){
