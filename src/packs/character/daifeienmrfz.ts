...
character("daifeienmrfz", { pack: "epicSJZX", sex: "female", group: "weimrfz", hp: 3, skills: ["zhuofengmrfz", "qianggongmrfz"] });
	...
	qianggongmrfz: {
		...
		async content(event, trigger, player) {
			...
			if (trigger.name == "useCard") {
				...
				let resetSkills: string[] = [];
				let suffixs = ["used", "round", "block", "blocker"];
				for (let skill of skillx) {
					let info = get.info(skill);
