...
skill({
  "lvmaimrfz": {
    ...
    zhuanhuanji(player, skill2) {
      if (player.storage[skill2] < 3) player.storage[skill2]++;
      else player.storage[skill2] = 0;
    },
    init(player, skill2) {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseCard().set("prompt", get.prompt("lvmaimrfz")).set("prompt2", `你可以重铸一张牌,然后若你手牌中最多的花色为${get.translation(lib.skill.lvmaimrfz.transfer(player))},你摸一张牌`).set("filterCard", (card) => {
    ...
    async content(event, trigger, player) {
      ...
      if (player.countCards("h", { suit }) === max) {
        await player.draw();
  ...
  "tiaoxiemrfz": {
    ...
    filter(event, player) {
      return player.getCards("h", (card) => {
        ...
        return player.countCards("h", (card2) => get.suit(card2) !== suit) > 0 && event.filterCard(card, player, event);
      }).length > 0;
    },
    filterCard(card) {
      let player = get.player(), event = get.event();
      let suit = get.suit(card);
      ...
      if (ui.selected.cards.length < 1) {
        ...
        return player.countCards("h", (card2) => get.suit(card2) !== suit) > 0 && ["basic", "trick"].includes(get.type(card)) && !player.getStorage("tiaoxiemrfz").includes(suit);
      }
    ...
    async content(event, trigger, player) {
      ...
      if (Object.keys(event.getParent("phaseUse")).length > 0 && event.getParent("phaseUse").player === player) await player.chooseUseTarget({ name }, [event.cards[1]]).set("forced", true);
      else {
        if (name === "wuxie") event._trigger = event.getParent(2)._trigger;
        const result = await player.useCard({ name }, [event.cards[1]]).set("forced", true).forResult();
