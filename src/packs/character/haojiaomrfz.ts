...
character("haojiaomrfz", { pack: "legendSJZX", sex: "female", group: "weimrfz", hp: 4, skills: ["xuezhanmrfz", "dunpaomrfz", "biaohaomrfz"] });
						...
						await player.chooseUseTarget(
							{
								...
								prompt: "请选择【杀】的目标 (【杀】:" + (num < 4 ? num : 3) + "/3;【万箭齐发】:0/1)",
								addCount: false,
							}
						);
						//@ts-ignore
	...
	xuezhanmrfz: {
		...
		async content(event, trigger, player) {
			...
			await player.chooseToDiscard({
				...
				forced:true,
				selectCard() {
					return get.player().countCards("hej");
				},
			});
			trigger.cancel();
			game.log(player, "复活");
