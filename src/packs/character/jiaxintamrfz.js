  ...
  zhixingmrfz: {
    ...
    subSkill: {
      draw: {
        ...
        async content(event, trigger, player2) {
          ...
          for (const target of targets) {
            const result = await target.chooseToDiscard().set("prompt", "【咫行】：制衡1（弃置一张牌，然后摸一张牌）").set("ai", (card) => 6 - get.value(card)).forResult();
            if (result.bool) {
      ...
      reset: {
        ...
        async content(event, trigger, player2) {
          let skill2 = get.sourceSkillFor(trigger);
          console.log(player2.awakenedSkills.includes(skill2), skill2, player2.awakenedSkills);
          player2.when({
            global: `${trigger.skill}After`
          }).step(async (event2, trigger2, player) => {
