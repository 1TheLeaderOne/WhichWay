    ...
    subSkill: {
      ...
      gain: {
        ...
        async content(event, trigger, player) {
          ...
          const result = await target.chooseControl(list).set("prompt", "选择一项").set("ai", function(player2) {
            return 0;
          }).forResult();
          ...
          if (result.control == "交牌") {
            ...
            if (resultx.bool && resultx.cards && resultx.cards.length) {
              if (resultx.cards.length == 1) {
                ...
                await player.chooseButton(["选择获得其中的一张牌", result.cards], true).set("ai", (button) => get.value(button.link)).forResult();
