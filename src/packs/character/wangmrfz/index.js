...
translate({
  ...
  dingshengmrfz: "定胜",
  dingshengmrfz_info: "限定技，出牌阶段，你可以从牌堆或弃牌堆中获得本局游戏你使用过的普通锦囊牌各一张，且令其他角色从牌堆或弃牌堆中获得你未因此获得的一张普通锦囊牌。"
});
let yearsMap = {};
const years = {
  shuomrfz: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player.chooseTarget().set("forced", true).set("prompt", `朔：对一名目标角色造成一点伤害`).set("prompt2", whichWayUtil.colorize(`#r劲发江潮落，气收秋毫平！#`)).set("filterTarget", (card, player2, target2) => {
        return targets.includes(target2);
      }).set("ai", (target2) => get.damageEffect(target2, player, player)).forResult();
  ...
  wangmrfz: {
    ...
    async content(event, trigger, player) {
      await player.chooseUseTarget({ name: "sha", nature: "stab", isCard: true }).set("prompt", `望：视为使用一张刺【杀】`).set("prompt2", whichWayUtil.colorize("#r眼观全局，胜负不在一处。#")).set("addCount", false).set("nodistance", true);
    }
  },
  lingmrfz: {
    audio: "shixingmrfz",
    ...
    filter(event, player, name, target) {
      const names = Array.from(
        new Set(
          ...
          }).map((evt) => evt.card.name).flat()
        )
      );
      return names.filter((name2) => player.hasUseTarget(name2, true, false)).length > 0;
    },
    async content(event, trigger, player) {
      const names = Array.from(
        new Set(
      ...
      player.chooseUseTarget({ name, isCard: true }).set("forced", true).set("prompt", `颉：视为使用一张你本回合使用过的基本牌`).set("prompt2", whichWayUtil.colorize("#r就这样吧#")).set("addCount", false).set("nodistance", true);
    }
  },
  shumrfz: {
    marktext: "黍",
  ...
  yimrfz: {
    ...
    async content(event, trigger, player) {
      const result = await player.chooseToDiscard().set("prompt", `易：${get.poptip("sjzx_zhiheng")}3`).set("prompt2", whichWayUtil.colorize("#r纵横捭阖，自有制衡之道。#")).set("ai", (card) => get.value(card, player) < 6).set("selectCard", [0, 3]).set("position", "he").forResult();
      if (result.cards) {
  ...
  nianmrfz: {
    ...
    async content(event, trigger, player) {
      const skill2 = getVaildEquipSkills(player).randomGet();
      player.addTempSkill(skill2, { player: "phaseEnd" });
      game.log(player, `获得 ${get.translation(skill2)} 直到回合结束`);
    }
  },
  ximrfz: {
    audio: "huijuanmrfz",
  ...
  yumrfz: {
    audio: "qizaomrfz",
    async content(event, trigger, player) {
      const result = await player.chooseControl("basic", "trick", "equip").set("prompt", "余：选择从牌堆中获得任意类型的一张牌。").set("prompt2", whichWayUtil.colorize("#r热锅冷油，火候正好#")).set("ai", function() {
        const player2 = get.player();
        if (player2.hp <= 3 && !player2.countCards("h", { name: ["shan", "tao"] })) {
          return "basic";
        }
        if (player2.countCards("he", { type: "equip" }) < 2) {
          return "equip";
        }
        return "trick";
      }).forResult();
...
for (const key in years) {
  ...
  info.filter = (event, player, name, target) => {
    ...
    return originalFilter ? originalFilter.bind(void 0)(event, player, name, target) : true;
  };
}
skill({
  qushimrfz: {
...
function getVaildEquipSkills(player) {
  ...
  for (let equipName of equips) {
    ...
    info.skills.forEach((skill2) => {
      if (!playerSkills.includes(skill2) && !skills.includes(skill2)) skills.push(skill2);
    });
  }
  return skills;
}
function getUsedTricks(player) {
