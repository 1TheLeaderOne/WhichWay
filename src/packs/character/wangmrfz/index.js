...
const years = {
  ...
  wangmrfz: {
    ...
    async content(event, trigger, player) {
      await player.chooseUseTarget().set("card", { name: "sha", nature: "stab", isCard: true }).set("prompt", `望：视为使用一张刺【杀】`).set("prompt2", whichWayUtil.colorize("#r眼观全局，胜负不在一处。#")).set("addCount", false).set("nodistance", true);
    }
  },
  lingmrfz: {
    audio: "shixingmrfz",
  ...
  jiemrfz: {
    ...
    async content(event, trigger, player) {
      ...
      player.chooseUseTarget().set("card", { name, isCard: true }).set("forced", true).set("prompt", `颉：视为使用一张你本回合使用过的基本牌`).set("prompt2", whichWayUtil.colorize("#r就这样吧#")).set("addCount", false).set("nodistance", true);
    }
  },
  shumrfz: {
    marktext: "黍",
  ...
  yumrfz: {
    ...
    async content(event, trigger, player) {
      ...
      }).set("prompt", "余：选择从牌堆中获得任意类型的一张牌。").set("prompt2", whichWayUtil.colorize("#r热锅冷油，火候正好#")).set("ai", function() {
        const player2 = get.player();
