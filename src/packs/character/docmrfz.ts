...
character("docmrfz", { pack: "epicSJZX", sex: "male", group: "othermrfz", hp: 4, skills: ["yizhemrfz", "zhulimrfz"] });
	...
	zhulimrfz: {
		...
		async content(event, trigger, player) {
			if (player.getStat("damage") != undefined) {
				if (player.countCards("he") > 0) player.chooseToDiscard({
					position:"he",
					forced:true,
					prompt:"【医者】:请选择弃置一张牌"
				});
				...
				if (!cards) return;
				for (let i of cards) {
					let num = get.number(i);
					if (get.suit(i) == "heart" && typeof num === "number" &&num >= 2 && num <= 9) {
