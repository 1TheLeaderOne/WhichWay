  ...
  "newjuntongmrfz": {
    ...
    mod: {
      targetInRange: function(card, player, target, now) {
        if (!player.storage.newjuntongmrfz.includes(get.name(card)) && (get.type(card) === "basic" || get.isSingle(card) && get.type(card) === "trick"))
          return true;
      },
      selectTarget: function(card, player, range) {
        if (!player.storage.newjuntongmrfz.includes(get.name(card)) && range[1] != -1 && (get.type(card) === "basic" || get.isSingle(card) && get.type(card) === "trick"))
          range[1]++;
      }
    }
  },
  "newpochengmrfz": {
    ...
    async cost(event, trigger, player) {
      ...
      if (trigger.name === "useCard") {
        ...
        const { control } = await player.chooseControl(list).set("prompt", prompt).set("ai", () => {
          let list2 = get.event().list, card = get.event().card, targets = get.event().targets, player2 = get.player();
          if (!list2.includes("额外目标") || !game.hasPlayer((current) => get.attitude2(current) < 0 && !targets.includes(current) && !!player2.canUse(card, current)))
            return "额外结算";
          if (targets.some((i) => i.hp < 2)) return "额外结算";
          return Math.random() > 0.5 ? "额外结算" : "额外目标";
        }).set("list", list).set("targets", trigger.targets).set("card", trigger.card).forResult();
        if (control === "cancel2") return event.result.bool = false;
          ...
          const { targets } = await player.chooseTarget(true).set("prompt", `请额外选择一个目标`).set("filterTarget", (card, player2, target) => {
            let targets2 = get.event().targets, cardx = get.event().cardx;
            return player2 != target && player2.canUse(cardx, target) && !targets2.includes(target);
          }).set("ai", (target) => get.attitude2(target) < 0).set("targets", trigger.targets).set("cardx", trigger.card).forResult();
          event.result = {
            bool: true,
            cost_data: targets
          };
        ...
        const { control } = await player.chooseControl("cancel2").set("prompt", `请选择一个选项`).set("choiceList", ["发动一次【军统①】", "重置【军统②】"]).set("ai", () => {
          let player2 = get.player();
          return player2.storage.newjuntongmrfz.length > 4 ? "选项二" : "选项一";
        }).forResult();
        event.result = {
          bool: true,
          //@ts-ignore
...
//# sourceMappingURL=fengdimrfz.js.map
