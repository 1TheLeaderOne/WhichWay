...
skill({
  "zhengrongmrfz": {
    ...
    filter: function(event, player) {
      let list = [], storage = player.storage.zhengrongmrfz;
    ...
    async content(event, trigger, player) {
      ...
      if (storage["losedraw"] == false) list.push("摸牌阶段摸牌数-1");
      if (storage["maxhp"] == false) list.push("失去体力上限");
      const result = await player.chooseControl({ controls: list }).set("ai", function() {
        return 0;
  ...
  "siyanmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      if (result?.control) {
        const target2 = trigger.targets[0];
        if (result.index === 1) {
          game.log(get.translation(target2), "选择了弃置两张手牌");
          await target2.chooseToDiscard({
            forced: true,
            selectCard: [2, 2]
          });
        } else {
