...
skill({
  ...
  "newjingsimrfz": {
    ...
    async content(event, trigger, player) {
      ...
      trigger.card;
      const result = await target.chooseBool("【警司】：是否请求将此" + get.translation(trigger.card) + "的目标改为" + get.translation(player) + "?").set("ai", function() {
        var player2 = _status.event.player, target2 = _status.event.getTrigger().player;
        return get.attitude(player2, target2) > 2;
      }).forResult();
      if (result.bool) {
...
//# sourceMappingURL=chenmrfz.js.map
