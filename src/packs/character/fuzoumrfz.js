...
let needDiscards = window.whichWaySave.tmpSave.hengzoumrfz_discard;
skill({
  hengzoumrfz: {
    audio: ["作战中3", "作战中4"],
    forced: true,
    ...
    filter(event, player, name, target) {
      return player.getHistory("lose", (evt) => event.cards.map((i) => i.cardid).some((id) => evt.gaintag_map?.[id]?.includes("visible_mutuanmrfz"))).length > 0;
    },
    async content(event, trigger, player) {
      const card = get.cardPile((card2) => get.color(card2) === "black");
      if (card) {
  ...
  mutuanmrfz: {
    ...
    async content(event, trigger, player) {
      ...
      await player.draw();
      const skill2 = lib.skill.mutuanmrfz;
      if (!lib.config.autoskilllist.includes("mutuanmrfz") && skill2.filter(event, player)) {
        const next = game.createEvent("mutuanmrfz");
        next.player = player;
        next.cards = player.getCards("h").filter((i) => skill2.filterCard(i));
        next.setContent(skill2.content);
      }
    },
    ai: {
      order: 13,
