...
skill({
  xiaoshimrfz: {
    ...
    async cost(event, trigger, player) {
      ...
      if (adjoins.length < 2) {
        const result2 = await player.chooseBool().set("prompt", `【霄式】:是否视为使用一张${get.translation(adjoins[0])}(无距离次数限制)？`).set("ai", () => {
    ...
    async content(event, trigger, player) {
      ...
      const { cards: [card] = [] } = await player.chooseUseTarget().set("card", { name, nature }).set("forced", true).set("addCount", false).set("nodistance", true).forResult();
      if (!card) return;
...
function getDifferent(player, getNum = false) {
  ...
  for (const evt of events) {
    if (simples.some((simple) => simple.name === evt.card.name && simple.nature === evt.card.nature)) {
      const index = simples.findIndex((simple) => simple.name === evt.card.name && simple.nature === evt.card.nature);
      if (index !== -1) {
