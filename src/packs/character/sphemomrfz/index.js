  ...
  "renbenmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      while (event.num < game.players.length) {
        result = await game.players[event.num].chooseButton(true, ["【人本】:请声明一张牌</br>科学理应注视每一个人", [event.cards, "vcard"]]).set("ai", (button) => {
      ...
      while (event.num2 < game.players.length) {
        ...
        if (currentPlayer !== player) {
          result = await currentPlayer.chooseControl("是", "否").set("prompt", "【人本】:是否遵守协议？(不能使用或打出" + get.translation(player.storage.renbenmrfz) + ")").set("ai", () => {
