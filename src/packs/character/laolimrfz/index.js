...
skill({
    ...
    subSkill: {
      anti: {
        ...
        filter: function(event, player) {
          ...
          return player.countCards("he") >= 2 && event.source != void 0;
        },
        logTarget: "source",
        async content(event, trigger, player) {
          const result = await player.chooseToDiscard("he", false, 2, "你可以弃置两张牌将此伤害转移给" + get.translation(trigger.source)).forResult();
          if (result.cards) {
            trigger.cancel();
      ...
      skip: {
        ...
        async content(event, trigger, player) {
          let result;
          result = await player.chooseToDiscard("h", false, 2, "你可以弃置两张牌并移动一张你判定区内的一张牌").set("ai", (card) => {
          ...
          if (result.links && result.links.length) {
            ...
            result = await player.chooseTarget(true, "选择" + get.translation(event.card) + "的移动目标", (card, player2, target) => {
              return target !== player2 && target.canAddJudge(event.card);
            }).forResult();
          } else {
            return;
          }
  ...
  "mingshimrfz": {
    ...
    async content(event, trigger, player) {
      const { target } = event;
      let result;
      result = await player.chooseCard("he", "请选择一张牌交给" + get.translation(target), true).set("ai", (card) => {
        return 6 - get.value(card);
      }).forResult();
      ...
      if (target.countCards("h") > 3) {
        ...
        result = await player.chooseControl("令其摸一张", "令其摸两张").set("prompt", "【明事】:请选择一项").set("prompt2", "你可以令其摸[2/1]张牌，然后于下一个准备阶段弃置[1/2]张牌").set("ai", () => {
  ...
  "jixiongmrfz": {
    ...
    async content(event, trigger, player) {
      let result;
      while (true) {
        result = await trigger.player.judge((card) => {
  ...
  "jixiongmrfz2": {
    ...
    direct: true,
    async content(event, trigger, player) {
      const result = await player.chooseControl("确定", "取消").set("prompt", "是否发动【吉凶】？").set("ai", function() {
