...
character("anjielinamrfz", { pack: "legendSJZX", sex: "female", group: "xumrfz", hp: 3, skills: ["xinshimrfz", "fanzhongmrfz"] });
	...
	fanzhongmrfz: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget({
					prompt: "是否发动【反重】?",
					prompt2: "你可以选择攻击范围内的一名其他角色，令其获得‘反重’标记",
		...
		async content(event, trigger, player) {
			...
			if (typeof mark !== "number") {
				...
				if (target.countCards("h") > 0) {
					result = await target.chooseCard({ forced: true }).forResult();
				...
				result = await player
					.chooseTarget(
						{
							forced: true,
							prompt: "【信使】:请选择一名其他角色，令其获得‘反重’标记",
							filterTarget(card, player, target) {
				...
				async content(event, trigger, player) {
					...
					while (player.countMark("xinshimrfz_give") >= 2) {
						...
						if (result.control !== "cancel2") {
							...
							if (card) {
								const next = player.gain({cards:[card],animate:"gain"});
