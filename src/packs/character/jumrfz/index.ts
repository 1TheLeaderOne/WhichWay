...
translate({
	...
	qiaoqimrfz: "巧器",
	qiaoqimrfz_info:
		"当你于一回合内首次使用一种花色的牌结算完毕后，你可以将此牌至于牌堆或弃牌堆顶或底，然后若弃牌堆和牌堆数的个位数相等，你摸一张牌。",
	feigongmrfz: "非攻",
	feigongmrfz_info:
		"锁定技，结束阶段，你依次展示牌堆顶、牌堆底、弃牌堆顶、弃牌堆底各一张牌，并获得其中红色的牌，然后若你没有因此获得牌，你失去一点体力。",
});
...
skill({
	feigongmrfz: {
		...
		async content(event, trigger, player) {
			...
			//@ts-ignore
			const copys: Card[] = Object.values(piles).slice(0);

			await player
				.showCards(copys, `${get.translation(player)}因【非攻】而展示了牌`)
	...
	qiaoqimrfz: {
		...
		async cost(event, trigger, player) {
			...
			const result = await player
				.chooseControl("牌堆顶", "牌堆底", "弃牌堆顶", "弃牌堆底", "cancel2")
				.set(
					"prompt",
				...
				.set("ai", () => {
					...
					if (player.hasSkill("feigongmrfz") && _status.currentPhase === player) {
						const piles = {
							牌堆顶: _status.pileTop,
							牌堆底: ui.cardPile.childNodes[ui.cardPile.childNodes.length - 1],
							弃牌堆顶: ui.discardPile.childNodes[0],
						...
						//@ts-ignore
						const hasRed = Object.values(piles).some(card => get.color(card) === "red");
						// 算鸟，ai开个挂怎么了
						// if(player.hasKnownCards(player,card=>card === pile.top)){}
						if (
							get.value(card) < 8 &&
							get.color(card) === "black" &&
							![1, -1].includes(ui.cardPile.childNodes.length - ui.discardPile.childNodes.length)
						)
							return "cancel2";
...
function getSingleDigit(num: number | string): number {
