...
skill({
  "yuanmengmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      if (target === player) {
        result = await player.chooseControl("确定", "cancel2").set("prompt", get.prompt("yuanmengmrfz")).set("prompt2", "你可以对" + get.translation(source) + "使用【杀】").set("ai", () => {
      ...
      if (!skipToStep2) {
        if (result && result.index === 0) {
          if (player.hasSha()) {
            await player.chooseToUse(true, function(card, player2, event2) {
      ...
      result = await player.chooseControl(get.translation(target), get.translation(source), "cancel2").set("prompt", get.prompt("yuanmengmrfz")).set("prompt2", "你可以对其中一名角色使用【杀】").set("ai", () => {
        const aiPlayer = _status.event.player;
        const aiTarget = _status.event.getTrigger().player;
  ...
  "ningshenmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      if (player.countMark("ningshenmrfz") >= 2) {
        result = await player.chooseControl("摸牌", "获得杀").set("prompt", get.prompt("ningshenmrfz")).set("prompt2", "请选择一项").set("ai", (event2, player2) => {
          if (player2.hp <= 2 || player2.hasCard((card) => {
...
//# sourceMappingURL=yuanyamrfz.js.map
