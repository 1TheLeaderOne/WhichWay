...
character("baitiemrfz", { pack: "legendSJZX", sex: "male", group: "weimrfz", hp: 4, skills: ["jigongmrfz", "jiefeimrfz"] });
...
skill({
	jigongmrfz: {
		...
		async content(event, trigger, player) {
			...
			const next = player
				.chooseControl()
				.set("choiceList", ["白铁多功能平台 - 攻击型：当你造成至少两点伤害时，你可以令此伤害 +1。", "白铁多功能平台 - 支援型：锁定技，弃牌阶段开始时，你摸一张牌并额外执行一个出牌阶段。", "铁钳号原型机：出牌阶段你可以弃置 X 张带有伤害类标签的牌，然后选择一名你攻击范围内的其他角色，对其造成一点伤害（X=此技能本回合使用数 +1）。"])
				.set("ai", () => {
		...
		subSkill: {
			gcard: {
				...
				filter: function (event, player) {
					let list = ["baitiemrfzcard1", "baitiemrfzcard2", "baitiemrfzcard3"];
					for (let i = 0; i < 3; i++) {
				...
				async content(event, trigger, player) {
					...
					result = await player
						.chooseTarget({
							prompt: "你可以将【支援装备】移动至一名其他角色的装备区",
							filterTarget(card, player, target) {
								return target !== player && !target.getEquip(5) && !target.hasEnabledSlot(5);
							},
						})
						.set("ai", target => {
			...
			zb: {
				...
				async content(event, trigger, player) {
					...
					const next = player
						.chooseControl()
						.set("choiceList", ["白铁多功能平台 - 攻击型：当你造成至少两点伤害时，你可以令此伤害 +1。", "白铁多功能平台 - 支援型：锁定技，弃牌阶段开始时，你额外执行一个出牌阶段和摸牌阶段。", "铁钳号原型机：出牌阶段你可以弃置 X 张带有伤害类标签的牌，然后选择一名你攻击范围内的其他角色，对其造成一点伤害（X=此技能本回合使用数 +1）。"])
						.set("ai", () => {
							return [0, 1, 2].randomGet();
						});
					next.set("prompt", get.prompt("jigongmrfz")).set("prompt2", "请选择一项");
				...
				async content(event, trigger, player) {
					...
					for (let i = 0; i < 3; i++) {
						if (trigger.player.countCards("e", list[i])) {
							let cards = trigger.player.getCards("e", function (card) {
								return card.name == list[i];
							});
	...
	jiefeimrfz: {
		...
		async content(event, trigger, player) {
			...
			// step 1
			result = await player.chooseBool({ prompt: get.prompt("jigongmrfz") }).forResult();
			...
			if (result.color === "red") {
				...
				result = await player
					.chooseTarget(
					{
						prompt:"你可以令一名角色装备【支援装备】",
						filterTarget(card, player, target) {
							return !target.getEquip(5) && !target.hasEnabledSlot(5);
						},
					}
					)
					.set("ai", target => {
