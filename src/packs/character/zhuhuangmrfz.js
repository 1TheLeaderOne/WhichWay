  ...
  "liaoyuanmrfz": {
    ...
    subSkill: {
      recorder: {
        ...
        async content(event, trigger2, player) {
          if (trigger2.name === "useCard") {
            delete player.storage.liaoyuanmrfz[trigger2.card.storage.randomId];
          } else {
      ...
      after: {
        ...
        async content(event, trigger2, player) {
          let showCards = player.storage.liaoyuanmrfz[trigger2.card.id].filter((card) => get.position(card) === "h");
          ...
          if (player.isUnderControl(true) && !_status.auto) {
            const { targets, cards } = await player.chooseCardTarget({
              ...
              filterTarget(card, player2, target) {
                return player2.canUse("wanjian", target);
              },
            ...
            let cards = showCards, targets = game.filterPlayer((i) => get.attitude(player, i) < 0);
            cards.forEach((card) => {
