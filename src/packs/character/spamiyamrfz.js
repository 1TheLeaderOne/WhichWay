...
skill({
  ...
  "newjueyingmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      let skills = player.getSkills(null, false, void 0).filter((skill2) => lib.translate[skill2 + "_info"]);
      let awakenedSkills = player.awakenedSkills.filter((skill2) => lib.translate[skill2 + "_info"]);
      ...
      skills.forEach((skill2) => {
        let translate2 = lib.translate[skill2 + "_info"];
        let matches = translate2.match(pattern);
        if (matches) {
