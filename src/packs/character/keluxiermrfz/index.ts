...
type Supports = Record<string, { audio: string; name: string; intro: string; code: string; content: (event: GameEvent, player: Player) => any; filter?: (event: GameEvent, player: Player) => boolean }>;

const stratagemSupport: {
	supports: Supports;
	codeFilter: (code: string, cards: Card[]) => boolean;
	getCodes: (cards: Card[]) => Array<any>;
	compileCode: (code: string) => string[];
	findMatchingCards: (cards: Card[]) => { cards: Card[]; name: string; intro: string }[];
				...
				const { targets } = await player
					.chooseTarget({
						...
						ai(target) {
							...
							const hdcount = target.countCards("h");
							if (hdcount <= 1) {
								num += 3;
							} else if (hdcount <= 2) {
								num += 2;
			...
			async content(event, player) {
				const result = await player
					.chooseTarget({
						...
						filterTarget(card, player, target) {
							return player.canUse("wanjian", target) && target !== player;
						},
						selectTarget: [1, 2],
						ai(target) {
							return get.attitude2(target) < 0 ? 1 : -1;
			...
			async content(event, player) {
				const result = await player
					.chooseTarget({
						...
						filterTarget(card, player, target) {
							return target !== player && player.canUse("wanjian", target);
						},
						selectTarget: [1, 3],
						forced: true,
						ai(target) {
				...
				if (result.targets?.length) {
					player
						.chooseUseTarget({
							...
							card: new lib.element.VCard({ name: "wanjian" }),
							filterTarget(card, player, target) {
								const targets: Player[] = get.event().targets;
								return target !== player && player.canUse("wanjian", target) && targets.includes(target);
							},
							selectTarget() {
								return get.event().targets.length;
							},
				...
				if (result.targets?.[0]) {
					...
					await player
						.chooseUseTarget({
							...
							card: new lib.element.VCard({ name: "wanjian" }),
							filterTarget(card, player, target) {
								const targetx = get.event().targetx as Player;
								return target !== player && player.canUse("wanjian", target) && targetx === target;
							},
						})
						.set("targetx", target);
				}
	...
	findMatchingCards(cards: Card[]): { cards: Card[]; name: string; intro: string }[] {
		...
		for (const card of cards) {
			const suit = get.suit(card) as string;
			if (!suitGroups[suit]) suitGroups[suit] = [];
			suitGroups[suit].push(card);
		}

		const results: { cards: Card[]; name: string; intro: string }[] = [];
		...
		for (const name in this.supports) {
			...
			let found = false;
			function backtrack(index: number, current: Card[]): void {
				if (found) return;
	...
	zhanbeimrfz: {
		...
		async cost(event, trigger, player) {
			...
			for (let info of codes) {
				tips += `${info.name}(${info.cards
					.slice(0)
					.map(c => get.translation(get.suit(c)))
					.join("")})：${info.intro}<br>`;
			}
			...
			event.result = await player
				.choosePlayerCard({
					...
					ai(button) {
						const code = get.event().code as { cards: Card[] };
						const card = button.link;
						if (get.suit(card) === get.suit(code.cards[ui.selected.buttons.length])) return 1145141919810 - get.value(card);
		...
		async content(event, trigger, player) {
			...
			for (let name in stratagemSupport.supports) {
				...
				if (stratagemSupport.codeFilter(support.code, cards) && (support.filter === undefined || support.filter(event, player))) {
