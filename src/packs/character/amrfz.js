    ...
    subSkill: {
      damage: {
        ...
        async content(event, trigger, player) {
          ...
          const result = await player.chooseTarget(true, "弃置一名角色区域内的一张牌", function(card, player2, target2) {
            ...
            return att * lib.card.guohe.ai.result.target(player2, target2);
          }).forResult();
          if (result.targets) {
  ...
  "qizhenmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      if (target.countCards("he") === 0) {
        ...
        result = await player.chooseControl(str1, str2).set("prompt", "【奇针】:请选择一项").set("ai", () => {
