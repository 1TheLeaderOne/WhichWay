...
skill({
	yuyimrfz: {
		...
		async cost(event, trigger, player) {
			...
			const result = await player
				.chooseControl({
					...
					ai(event, player) {
						const types = get.event().TYPES as string[];
						const val = {} as Record<string, Array<Card | VCard>>;
						...
						if (!player.hasSkill("yaojinmrfz")) {
							if (player.getExpansions("yuyimrfz").length > 0) return "cancel2";
							let type = "cancel2";
							let minVal = 0;
							for (let key in val) {
						...
						let maxNum = null as null | number;
						for (let key in val) {
		...
		async content(event, trigger, player) {
			...
			const { control } = await player
				.chooseControl({
					...
					ai(event, player) {
						if (!player.hasSkill("yaojinmrfz")) return "选项二";
		...
		onremove(player, type) {
			...
			if (cards.length > 0) {
				player.loseToDiscardpile({ cards: cards });
			}
		},
		intro: {
			...
			tao: {
				...
				async content(event, trigger, player) {
					...
					if (group < 2) {
						...
						const dialogAuto: any[] = ["【愈翼】:将一组牌当作【桃】使用", [control, "tdnodes"]];
						let i_tmp = 0;
						for (let key in taos) {
							const data = taos[key] as data;
...
function accumulateCardVal(arr: Array<Card | VCard>): number {
