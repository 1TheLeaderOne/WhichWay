...
character("fengdimrfz", { pack: "legendSJZX", sex: "female", group: "weimrfz", hp: 4, skills: ["newjuntongmrfz", "newpochengmrfz"] });
...
skill({
	newjuntongmrfz: {
		...
		mod: {
			targetInRange: function (card:Card, player, target, now) {
			...
			selectTarget: function (card:Card, player, range) {
	...
	newpochengmrfz: {
		...
		async cost(event, trigger, player) {
			...
			if (trigger.name === "useCard") {
				...
				const { control } = await player
					.chooseControl({controls:["选项一","选项二","cancel2"]})
					.set("prompt", `请选择一个选项`)
