...
character("baidurenmrfz", { pack: "epicSJZX", sex: "male", group: "mimrfz", hp: 4, skills: ["wenchoumrfz"] });
	...
	wenchoumrfz: {
		...
		async content(event, trigger, player) {
			...
			if (get.type(card) === "basic" && !get.tag(card, "damage")) {
				await player
					.chooseUseTarget({ card: get.autoViewAs({ name: "sha", isCard: true }) })
					.set("forced", true)
					.set("addCount", false)
					.set("nodistance", true)
					.set("prompt", "你视为使用一张无距离和次数限制的【杀】，然后此技能本回合失效")
					.set("prompt2", whichWayUtil.colorize(`#s血债血偿！#`));
