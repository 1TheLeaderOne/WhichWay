  ...
  "hongsongmrfz": {
    ...
    subSkill: {
      shan: {
        ...
        async content(event, trigger, player) {
          let result;
          result = await trigger.player.chooseControl("确定", "取消").set("prompt", get.prompt("hongsongmrfz")).set("prompt2", "你可以进行判定，若不为♥，其视为使用或打出一张【闪】并获得判定牌").forResult();
      ...
      give: {
        ...
        async content(event, trigger, player) {
          ...
          while (player.hasMark("hongsongmrfz")) {
            result = await player.chooseTarget(get.prompt("hongsongmrfz"), "你可以将任意个'红松'标记交给任意名其他角色", (card, player2, target) => {
...
//# sourceMappingURL=yanweimrfz.js.map
