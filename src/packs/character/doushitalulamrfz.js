...
skill({
  "talula_shixinmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      var { cards } = await player.choosePlayerCard("hes", trigger.player, true).set("prompt", `【拾薪】:请选择其各区域内的一张牌`).set("selectButton", pos.length).set("filterButton", (button) => {
        let cards2 = ui.selected.cards;
        let pos2 = cards2.slice().map((card) => get.position(card));
      ...
      while (cards.length > 0) {
        var { links } = cards.length == 1 ? { links: cards } : await player.chooseCardButton("【拾薪】:请选择要分配的牌", true, cards, [1, cards.length]).set("ai", () => {
        ...
        const { targets } = await player.chooseTarget("选择一名角色获得" + get.translation(event.togive), true).set("ai", (target) => {
  ...
  "zhuoximrfz": {
    ...
    filter(event, player) {
      ...
      return !player.storage.zhuoximrfz.includes(get.type2(event.card)) && game.hasPlayer((current) => {
        return get.distance(current, player) == player.hp && current != player;
      });
    },
    // @ts-ignore
    async cost(event, trigger, player) {
      const { result } = await player.chooseTarget(`【灼息】:你可以对一名与你距离为${player.hp}的角色造成一点火焰伤害`).set("filterTarget", (card, player2, target) => {
        return get.distance(target, player2) == player2.hp && target != player2;
      }).set("ai", (target) => {
        var player2 = get.event().player;
        return get.damageEffect(target, player2, player2, "fire") > 0;
      });
      if (!result) return;
      event.result = result;
    },
...
//# sourceMappingURL=doushitalulamrfz.js.map
