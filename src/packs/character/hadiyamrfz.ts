...
character("hadiyamrfz", { pack: "epicSJZX", sex: "female", group: "samrfz", hp: 4, skills: ["juetumrfz"] });
...
skill({
	juetumrfz: {
		...
		async content(event, trigger, player) {
			...
			if (winner !== player) player.damage({source:target});
			if (winner !== target) target.damage({source:player});
		},
		ai: {
			order: 5,
			result: {
