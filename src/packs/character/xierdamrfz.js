...
skill({
  "shencimrfz": {
    ...
    async content(event, trigger, player) {
      ...
      for (let name2 of lib.inpile) {
        ...
        cards.push(game.createCard(name2, list[0], list[1], list[3]));
      }
      const result = await player.chooseCardHand().set("ai", (card2) => get.player().getUseValue(card2)).set("cards", cards).set("forced", true).set("complexSelect", true).set("prompt", `【神赐】:请选择你要获得的牌`).forResult();
  ...
  "lanshengmrfz": {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseCard().set("prompt", get.prompt("lanshengmrfz")).set("prompt2", `你可以重铸至多${suits.size}张牌，若包含了与获得过的“神赐”牌相同牌名的牌，你摸一张牌`).set("filterCard", (card) => {
    ...
    async content(event, trigger, player) {
      ...
      let { cards } = event;
      player.recast(cards, void 0, (player2, cards2) => {
        if (cards2.map((i) => get.name(i)).some((i) => names.has(i))) {
          player2.draw(cards2.length + 1);
        } else player2.draw(cards2.length);
      });
    }
  }
});
