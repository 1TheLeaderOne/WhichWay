...
skill({
  "junqimrfz": {
    ...
    subSkill: {
      ...
      zhiyu: {
        ...
        async content(event, trigger, player) {
          let result;
          result = await player.chooseToDiscard("he", true, [1, 3], "请弃置至多三张牌").set("ai", (card) => {
            return 6 - get.value(card);
          }).forResult();
          if (result.cards) {
            result = await player.chooseTarget(true, [1, result.cards.length], "请选择至多" + result.cards.length + "名角色").set("ai", (target) => {
      ...
      zhiyuan: {
        ...
        async content(event, trigger, player) {
          ...
          if (trigger.player != player) {
            result = await player.chooseCard(true, "he", "交给" + get.translation(trigger.player) + "一张牌").set("ai", function(card) {
      ...
      get: {
        ...
        async content(event, trigger, player) {
          ...
          let result;
          result = await player.chooseControl("支援之旗", "治愈之旗", "进攻之旗").set("choiceList", [str1, str2, str3]).set("ai", (event2, player2) => {
      ...
      reget: {
        ...
        async content(event, trigger, player) {
          ...
          result = await player.chooseControl("确定", "cancel2").set("prompt", get.prompt("junqimrfz")).set("prompt2", "是否更换‘军旗’类型").forResult();
  ...
  "butuimrfz": {
    ...
    async content(event, trigger, player) {
      ...
      result = await trigger.player.chooseControl(list, "cancel2").set("prompt2", get.prompt("butuimrfz")).set("prompt", "请选择一项").set("ai", () => {
...
//# sourceMappingURL=qinliumrfz.js.map
