...
skill({
  "lingkongmrfz": {
    ...
    async cost(event, trigger, player) {
      ...
      while (count + 1 > 0) {
        const result = await player.chooseToUse().set("prompt", `【翎空】:请选择的你要使用的牌`).set("prompt2", `还需使用${count + 1}张牌即可将本阶段(${get.translation(trigger.phaseList[trigger.num])})改为出牌阶段`).forResult();
  ...
  "mijianmrfz": {
    ...
    async cost(event, trigger, player) {
      ...
      if (trigger.name === "damage") {
        event.result = await player.chooseBool().set("prompt", get.prompt("mijianmrfz")).set("prompt2", `你可以摸${count + 1}张牌`).set("ai", () => true).forResult();
      } else {
        event.result = await player.chooseCard("he").set("prompt", get.prompt("mijianmrfz")).set("prompt2", `你可以制衡${count + 1}`).set("selectCard", () => {
