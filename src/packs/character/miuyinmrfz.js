...
skill({
  miuyuanmrfz: {
    ...
    async content(event, trigger, player) {
      ...
      const { index } = opt2 ? await target.chooseControl({ choiceList: choices }).set("prompt", "【谬源】：请选择一项").set("ai", () => get.attitude(target, player) > 0 ? 0 : 1).forResult() : { index: 0 };
      ...
      if (index === 0) {
        ...
        if (target.countCards("h") >= 2) {
          const give = await target.chooseCard({
            prompt: "【谬源】：交给" + get.translation(player) + "两张手牌",
            position: "h",
            selectCard: [2, 2],
            ai: (card2) => -get.value(card2),
            forced: true
          }).forResult();
    ...
    subSkill: {
      zeng: {
        ...
        filter(event, player) {
          const buffs = player.storage.miuyuanmrfz || [];
        ...
        async content(event, trigger, player) {
          player.storage.miuyuanmrfz.remove(trigger.player);
          const { targets } = event;
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseTarget({
        ...
        filterTarget(card2, player2, target) {
          ...
          const usery = _status.event.usery;
      ...
      }).set("targetprompt", ["移除目标", "新增目标"]).set("cardx", card).set("usery", user).set("oldList", trigger.targets.filter((t) => t.isIn())).forResult();
    },
    async content(event, trigger, player) {
