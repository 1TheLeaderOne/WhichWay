...
character("geleidiyamrfz", { pack: "legendSJZX", sex: "female", group: "liemrfz", hp: 3, maxHp: 4, skills: ["quliemrfz", "newxunxiangmrfz", "xueshuomrfz", "tongmaimrfz"], clans: ["深海猎人"] });
...
skill({
	quliemrfz: {
		intro: {
			...
			mark: function (dialog, storage, player) {
				let cards = player.getExpansions("quliemrfz");
		...
		onremove(player) {
			game.countPlayer(current => {
				let cards = current.getExpansions("quliemrfz");
		...
		subSkill: {
			die: {
				...
				async content(event, trigger, player) {
					trigger.player.loseToDiscardpile({ cards: trigger.player.getExpansions("quliemrfz") });
				},
			},
			eff: {
				charlotte: true,
				...
				async content(event, trigger, player) {
					if (trigger.name == "phaseZhunbei") {
						...
						if (!links) return;
						if (links.length > 0) {
							current.gain({ cards: links, animate: "draw" });
							game.log(current, "收回了" + get.cnNumber(links.length) + "因【驱猎】而置于武将牌上的张牌");
						}
						if (cards.length != links.length) current.discard({ cards: cards.removeArray(links) });
						current.unmarkSkill("quliemrfz");
					} else {
						let current = trigger.player;
						let num = Math.ceil(current.countCards("h") / 2);
	...
	newxunxiangmrfz: {
		...
		filter(event, player) {
			let cards = lib.skill.zheqimrfz_eff2.getDiscard(event);
		...
		async content(event, trigger, player) {
			...
			if (result.bool) {
				let discards = lib.skill.zheqimrfz_eff2.getDiscard(trigger);
				const { links } =
