...
character("baijinmrfz", { pack: "epicSJZX", sex: "female", group: "kamrfz", hp: 3, skills: ["xujimrfz", "shiyumrfz"] });
	...
	xujimrfz: {
		...
		intro: {
			content: function (event, player) {
				let storage = player.storage.xujimrfz;
				let str;
		...
		filter: function (event, player) {
			return (
				player.getHistory("useCard", function (evt) {
					...
					if (evt.targets && evt.targets.length && evt.isPhaseUsing(player)) {
						let targets = evt.targets.slice(0);
		...
		async content(event, trigger, player) {
			...
			if (player.hasUseTarget("sha", false) && player.hasSha())
				result = await player
					.chooseControl({ controls: ["摸牌", "出杀"] })
					.set("prompt", "【蓄击】:请选择一项")
					.forResult();
			else {
				let num = 2 + player.storage.shiyumrfz_buff;
				player.draw(num || 2);
				event.finish();
			}
		...
		subSkill: {
			sha: {
				...
				filter: function (event, player) {
					...
					return event.card.name == "sha" && (evt && !evt.directHit.includes(event.target));
				},
				logTarget: "target",
				async content(event, trigger, player) {
					let storage = player.storage.shiyumrfz_buff;
