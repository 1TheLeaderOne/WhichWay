...
skill({
  ...
  "fuzhimrfz": {
    ...
    filter(event, player) {
      let skills = player.getSkills().filter((skill2) => {
        if (!skill2.startsWith("jingxiangmrfz")) return false;
    ...
    async content(event, trigger, player) {
      let name = player.getSkills().filter((skill2) => {
        if (!skill2.startsWith("jingxiangmrfz")) return false;
  ...
  "kuorongmrfz": {
    ...
    subSkill: {
      reset: {
        ...
        async content(event, trigger, player) {
          let skills = player.awakenedSkills.filter((skill2) => {
            return skill2.startsWith("jingxiangmrfz");
          });
          skills.forEach((skill2) => {
            if (player.awakenedSkills && player.awakenedSkills.includes(skill2)) {
              game.log(player, "重置了技能", `#g【${get.translation(skill2)}】`);
              player.awakenedSkills.remove(skill2);
            }
            if (player.disabledSkills) {
              for (let key in player.disabledSkills) {
                if (key === skill2) delete player.disabledSkills[key];
              }
            }
            lib.skill[skill2].usedSJZX = false;
          });
        }
      }
    }
...
//# sourceMappingURL=plot_keluxiermrfz.js.map
