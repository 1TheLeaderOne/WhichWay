...
let addRemoveTips = false;
function getCards(player) {
  player.storage.huozhimrfz ??= [];
  return player.storage.huozhimrfz;
}
function setCards(player, cards) {
  if (addRemoveTips === false) {
    player.when({ global: "phaseAfter" }).step(async (event, trigger, player2) => {
      addRemoveTips = false;
      const cards2 = [];
      game.players.forEach((p) => cards2.push(...p.getCards("hesx")));
      cards2.forEach((card) => whichWayTips.removePrompt(card, "huozhimrfz_effect"));
      player2.storage.huozhimrfz = [];
    });
  }
  addRemoveTips = true;
