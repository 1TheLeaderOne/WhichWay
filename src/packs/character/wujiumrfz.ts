...
import { character, characterIntro, characterTitle, skill, translate, dynamicTranslate } from "../hooks.ts";
...
// dynamicTranslate("tatumrfz",(player)=>{
//     return `连招技（非基本牌+【杀】+【${whichWayUtil.colorize(`#r${get.translation(player.getStorage("tatumrfz")[2]) || "杀"}#`)}】），当你使用的下一张牌结算完毕后，你将本技能描述中的红色部分替换为此牌的牌名;当你使用牌后，你可以中断此技能的连招以视为使用后项（无距离次数限制）。`
// });
			...
			for (let target of targets) {
				...
				const result = await target
					.chooseToDiscard({
						...
						ai(card) {
							const player = get.player(),
								target: Player = get.event().targetx;
...
function getTriggered(player: Player) {
	const source = player.getRoundHistory("damage", evt => evt.player === player && !!evt.source).map(evt => evt.source);
...
// function getLastUsed(player: Player, event: GameEvent, num: number = 1) {
// 	let history = player.getAllHistory("useCard");
// 	let index;
// 	if (event) {
// 		index = history.indexOf(event) - num;
// 	} else {
// 		index = history.length - num;
// 	}
// 	if (index >= 0) {
