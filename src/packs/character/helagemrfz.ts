...
character("helagemrfz", { pack: "legendSJZX", sex: "male", group: "wumrfz", hp: 4, skills: ["yingkuimrfz", "cangfengmrfz", "yuexiangmrfz"] });
...
skill({
	...
	cangfengmrfz: {
		...
		async content(event, trigger, player) {
			...
			let mark = player.countMark("cangfengmrfz");

			player.addMark("cangfengmrfz", trigger.num, false);

			if (mark / 2 >= 1 && player.getDamagedHp() > 0) {
				result = await player
					.chooseControl({ controls: ["摸牌", "回血"] })
					.set("prompt", "摸一张牌或回一点血")
					.forResult();
