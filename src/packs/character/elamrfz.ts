...
character("elamrfz", { pack: "legendSJZX", sex: "female", group: "othermrfz", hp: 3, skills: ["leimingmrfz", "zuzhimrfz"] });
			...
			ban: {
				...
				mod: {
					cardEnabled: function (card, player) {
						if (player.getStorage("zuzhimrfz_ban").includes(get.color(card as Card))) return false;
					},
	...
	leimingmrfz: {
		...
		async content(event, trigger, player) {
			...
			const { links } = await player
				.chooseButton({
					createDialog:[get.prompt("leimingmrfz"), [list1.concat(list2).concat(list3).concat(list4), "vcard"]]
				})
		...
		subSkill: {
			eff: {
				...
				onremove: true,
				filter(event, player) {
					let storage = player.storage.leimingmrfz,
						card = event.card;
				...
				async content(event, trigger, player) {
					...
					if (player.canUse({ name: "sha", nature: "thunder" }, target, false)) {
						let { control } = await player
							.chooseControl({controls:["red", "black"]})
