...
skill({
  ...
  "guiqiangmrfz": {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player.chooseToDiscard(get.prompt("guiqiangmrfz"), "你可以弃置一张牌并失去此技能，然后获得【鬼人】", "he").set("ai", function(card) {
  ...
  "luanwumrfz": {
    ...
    async content(event, trigger, player) {
      let result;
      result = await player.chooseTarget(true, "【乱舞】:请选择一名其他角色，视为对其使用一张结算三次的【杀】", (card, player2, target) => {
...
//# sourceMappingURL=spyedaomrfz.js.map
