  ...
  "xinshimrfz": {
    ...
    async content(event, trigger, player) {
      ...
      if (player.storage.xinshimrfz.length === 3) {
        result = await player.chooseTarget(true, "【信使】:请选择一名其他角色，令其获得‘反重’标记", (card, player2, target2) => {
      ...
      give: {
        ...
        async content(event, trigger, player) {
          ...
          while (player.countMark("xinshimrfz_give") >= 2) {
            result = await player.chooseControl("basic", "trick", "equip", "cancel2").set("prompt", "选择获得一种类型的牌").set("ai", () => {
...
//# sourceMappingURL=anjielinamrfz.js.map
