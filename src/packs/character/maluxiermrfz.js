...
skill({
        ...
        filter: function(event, player) {
          ...
          return player.hasCard(
            (card) => lib.inpile.some((name) => {
              ...
              if (event.filterCard({ name, isCard: true, cards: [card] }, player, event)) return true;
              return false;
            }, "hes")
          );
        },
  ...
  "cangshumrfz": {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseControl("选项一", "选项二", "cancel2").set("choiceList", [`摸${Math.min(player.getCardUsable("sha"), player.maxHp)}张牌`, `使用【杀】的次数+1`]).set("ai", () => {
