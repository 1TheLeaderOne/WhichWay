    ...
    zaowuzhu: {
      ...
      effect: {
        ...
        intro: [
          ...
          `·变更技能组：`,
          `【编译】`,
          () => get.skillInfoTranslation("bianyimrfz")
        ],
        content(player, info) {
          if (info) {
            info.hp = 4;
    ...
    hechuxuntuo: {
      ...
      effect: {
        ...
        intro: [
          `·变更技能组：`,
          `【千相】`,
          () => get.skillInfoTranslation("qianxianmrfz"),
          `【自悟】`,
          () => get.skillInfoTranslation("ziwumrfz"),
          `【遁空】(非初始技能)`,
          () => get.skillInfoTranslation("dunkongmrfz")
        ]
      }
    }
  }
};
const modulesList = Object.entries(modules).flatMap(
  ([key, subModules]) => Object.entries(subModules).map(([name, module]) => module.id ?? name)
);
export {
  modules,
  modulesList
