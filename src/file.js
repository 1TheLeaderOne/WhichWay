...
class WhichWayFile {
  ...
  async download(url, dir, filename, onprogress) {
    ...
    const wrappedOnProgress = onprogress ? (loaded, totalFromCordova) => {
  ...
  generateProgress(info, file, toastID) {
    ...
    if (percent !== null) {
      ...
      whichWayToast.showToast(`正在下载文件${file}: ${percent}%`, 3e3, "topRight", toastID);
    } else {
      if (whichWayUtil.isDeveloperMode()) {
        console.log(`正在下载文件 ${file}: ${loaded} bytes`);
      }
      whichWayToast.showToast(`正在下载文件 ${file}: ${Math.floor(loaded / 1024)} KB`, 3e3, "topRight", toastID);
    }
  }
  /**
...
const whichWayFile = new WhichWayFile();
onSetDev({
