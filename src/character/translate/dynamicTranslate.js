...
const dynamicTranslate = {
  tongmaimrfz: function(player) {
    let str = `你可以令一名深海猎人的角色回复一点体力或复原武将牌`, storage = player.storage.tongmaimrfz;
    if (!storage) storage = [];
  ...
  yirenmrfz: function(player) {
    return "蓄力技（<span class=thundertext>" + (player.countMark("charge") != void 0 ? player.countMark("charge") : 0) + "</span>/∞）。①锁定技，回合结束时，你增加X点蓄力值。（X=本回合出牌阶段剩余使用【杀】的次数）②出牌阶段，你可以消耗[<span class=thundertext> " + (player.storage.yirenmrfz_use != void 0 ? Math.min(player.storage.yirenmrfz_use, 5) : 1) + " </span>]点蓄力值，弃置两张手牌并选择至多两名其他角色，视为你对其各使用一张【杀】（目标必须合法且不计入次数限制）且其下个回合使用【杀】的次数-1，然后令[ ]中的数字+1（至多为5）。";
  },
