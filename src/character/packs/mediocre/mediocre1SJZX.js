  ...
  skill: {
    ...
    songgemrfz: {
      ...
      async content(event, trigger, player) {
        let cards = [];
        while (true) {
          const result = await player.judge().set("judge", (card) => {
            let { judgeSuits, judgeNames } = get.event();
            return judgeSuits.includes(get.suit(card)) || judgeNames.includes(get.name(card)) ? -1 : 4;
          }).set("judge2", (result2) => result2.bool).set("judgeSuits", cards.map((i) => get.suit(i))).set("judgeNames", cards.map((i) => get.name(i))).set("callback", async (event2) => {
            event2.parent?.orderingCards.remove(event2.card);
          }).forResult();
          if (result.card) cards.push(result.card);
          if (cards.map((i) => get.suit(i)).length !== new Set(cards.map((i) => get.suit(i))).size || cards.map((i) => get.name(i)).length !== new Set(cards.map((i) => get.name(i))).size) break;
          const { bool } = await player.chooseBool(get.prompt("songgemrfz")).set("frequentSkill", "songgemrfz").forResult();
          if (bool !== true) break;
        }
...
export {
  mediocre1SJZX as default
};
//# sourceMappingURL=mediocre1SJZX.js.map
