  ...
  skill: {
    ...
    wangqiongmrfz: {
      ...
      async content(event, trigger, player) {
        ...
        await player.chooseUseTarget({ name }).set("forced", true).set("prompt", `请选择【${get.translation(name)}】的目标`).forResult();
      }
    },
    tadimrfz: {
      audio: ["选中干员1", "选中干员2"],
    ...
    fulingmrfz: {
      ...
      async cost(event, trigger, player) {
        ...
        const result = await player.chooseControl("一", "二", "三", "cancel2").set("prompt", get.prompt("fulingmrfz")).set("prompt2", `你可以摸至多三张牌，然后你的回合结束时，若你执行选项的数量少于你摸的牌数，你失去X点体力。(X=你摸的牌数 - 你执行的项数)`).set("ai", () => {
    ...
    newkuangyumrfz: {
      ...
      async cost(event, trigger, player) {
        event.result = await player.chooseTarget().set("prompt", get.prompt("newkuangyumrfz")).set("prompt2", `你可以为此牌额外指定至多${get.cnNumber(player.hp)}个合法目标，若此牌没有造成伤害，你失去一点体力，反之，此技能本回合视为未发动过`).set("selectTarget", [0, player.hp]).set("filterTarget", (card, player2, target) => {
          let targets = get.event().targetsx;
          return !targets.includes(target) && player2.canUse(get.event().cardx, target);
        }).set("cardx", trigger.card).set("targetsx", trigger.targets).set("ai", (target) => {
          let val = 0, player2 = get.player(), card = get.event().cardx;
          if (get.attitude(target, player2) > 0) return -1;
      ...
      async content(event, trigger, player) {
        ...
        player.when({ player: "useCardAfter", source: "damageSource" }).filter((event2, player2) => {
          return event2.card && event2.card.cardid === trigger.card.cardid;
        }).step(async (event2, trigger2, player2) => {
          if (trigger2.name === "useCard") {
            player2.loseHp();
          } else {
            player2.draw();
            player2.setSkillCount("newkuangyumrfz", -1);
          }
        });
      }
    },
      ...
      subSkill: {
        hack: {
          ...
          async content(event, trigger, player) {
            ...
            let count = 0;
            let list = [];
            for (let i2 = 0; i2 < packs.length; i2++) {
              list.push([i2, getPacksTranslation(packs[i2])]);
            }
            ...
            for (let i2 = 0; i2 < sorts.length; i2++) {
              list_sorts.push([i2, lib.translate[sorts[i2]]]);
            }
            ...
            for (let name of characters) {
              if (get.character(name)?.skills) {
                get.character(name).skills.forEach((i2) => player.addSkill(i2));
              }
            }
    ...
    lingxinmrfz: {
      ...
      forced: true,
      async content(event, trigger, player) {
        player.when({ global: [`${trigger.skill}After`, `${trigger.skill}_costAfter`] }).filter((event2) => {
          console.log(event2);
          return !(event2.name.endsWith("_cost") && event2.result?.bool === true);
        }).then(() => {
        }).assign({
          ai: {
            viewHandcard: true,
            skillTagFilter(player2, tag, arg) {
              if (player2 == arg) return false;
            }
          }
        });
      },
        ...
        addTrigger: {
          ...
          async content(event, trigger, player) {
            ...
            game.broadcastAll(
              //@ts-ignore
              (skills2, player2) => {
                lib.skill.lingxinmrfz.trigger.player.addArray(skills2);
                player2.addSkillTrigger("lingxinmrfz", null, true);
              },
              skills,
              //@ts-ignore
              player
    ...
    lianganmrfz: {
      ...
      async cost(event, trigger, player) {
        ...
        event.result = await player.choosePlayerCard(target).set("prompt", get.prompt("lianganmrfz")).set("prompt2", `你可以连接${get.translation(target)}的一张手牌`).set("ai", (button) => get.value(button.link)).forResult();
      },
      async content(event, trigger, player) {
      ...
      subSkill: {
        view: {
          ...
          hiddenCard(player, name) {
            return game.getConnectCards().map((card) => get.name(card)).includes(name);
          },
          chooseButton: {
            dialog(event, player) {
    ...
    qinyinmrfz: {
      ...
      async content(event, trigger, player) {
        ...
        const { cards } = await player.choosePlayerCard(target, true).set("prompt", `请以弃置的方式将${get.translation(target)}的一张手牌当【顺手牵羊】使用`).set("nodistance", true).set("ai", (button) => {
      ...
      subSkill: {
        roundEnd: {
          ...
          async cost(event, trigger, player) {
            event.result = await player.chooseTarget().set("prompt", get.prompt("qinyinmrfz")).set("filterTarget", lib.skill.qinyinmrfz.filterTarget).set("ai", (target) => get.attitude2(target) < 0).set("nodistance", true).forResult();
          },
          async content(event, trigger, player) {
    ...
    gongximrfz: {
      ...
      enable: "phaseUse",
      filter(event, player) {
        if (Object.entries(player.getStat("skill")).filter((i) => ["gongximrfz", "gongximrfz_dying"].includes(i[0])).reduce((n, arr) => n + arr[1], 0) > 0)
          return false;
      ...
      check(card) {
        let player = get.player();
        return 8 - get.value(card) + Math.min(
      ...
      async content(event, trigger, player) {
        ...
        if (player.hasUseTarget("huoshaolianying")) {
          player.when({ player: "useCardAfter" }).filter((event2) => event2.card?.storage?.gongximrfz === true).step(async (event2, trigger2, player2) => {
            const damages = player2.getHistory("sourceDamage", (evt) => evt.card && evt.card.storage && evt.card.storage.gongximrfz === true).length;
            if (damages > 0) {
              const { targets: targets2 } = await player2.chooseTarget().set("selectTarget", [0, damages]).set("prompt", `你可以令至多${damages}名角色回复一点体力并摸一张牌`).set("ai", (target) => {
                let player3 = get.player();
                let att = get.attitude(player3, target);
                let num = att > 0 ? 1 : -114514;
                if (att > 0 && target.hp < 2) num += 10;
                if (target === player3) num += 2;
                if (att > 0 && target.getDamagedHp() > 0) num += 2;
                return num;
              }).forResult();
              if (targets2) {
                player2.logSkill("gongximrfz", targets2);
          ...
          await player.chooseUseTarget({ name: "huoshaolianying", storage: { gongximrfz: true } }, [card]).set("forced", true).set("prompt2", `此牌结算完成后，你令至多Y名角色回复一点体力并摸一张牌。（Y=【火烧连营】此次造成的伤害值）`);
        }
      },
...
export {
  legend3SJZX as default
};
//# sourceMappingURL=legend3SJZX.js.map
