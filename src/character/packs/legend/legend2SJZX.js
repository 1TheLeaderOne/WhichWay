  ...
  skill: {
            ...
            switch (colorx) {
              ...
              case "black":
                player.when({
                  ...
                  return event2.card.cardid === trigger.card.cardid;
                }).step(async (event2, trigger2, player2) => {
                  if (trigger2.name === "phase") return;
                  player2.removeSkill("lieqiongmrfz_enchanting_liegong");
                });
      ...
      init(player, skill) {
        ...
        game.broadcastAll((skills) => {
          skills.forEach((skill2) => {
            ...
            lib.dynamicTranslate[skill2] = function(player2) {
              if (!player2.storage.wuweimrfz || lib.skill.wuweimrfz.getNum(player2, skill2) < 1) return info;
          ...
          lib.dynamicTranslate["wuweimrfz"] = function(player2) {
            ...
            for (let skill2 of skills) {
              if (lib.skill.wuweimrfz.getNum(player2, skill2) > 0)
        ...
        fail: {
          ...
          async content(event, trigger, player) {
            ...
            for (let i = 0; i < 5; i++) {
              ...
              await player.chooseUseTarget({ name: "sha", isCard: true }).set("filterTarget", (card, player2, target) => {
    ...
    gandianmrfz: {
      ...
      async content(event, trigger, player) {
        ...
        const { control } = await player.chooseControl("red", "black").set("prompt", "【感电】：请选择令一种颜色的牌称之为“小轴”，其余颜色的牌则称之为“小树”").set("ai", () => {
          return "red";
        }).forResult();
        ...
        const result = await player.chooseCardTarget({
          ...
          ai1(card) {
            ...
            return 6 - get.value(card, player2);
          },
          ai2(target) {
            let player2 = get.player();
            if (target === player2) return 114514;
      ...
      async content(event, trigger, player) {
        ...
        if (!player.storage.newyanxunmrfz_addTempSkill) {
          game.broadcastAll((player2) => {
            player2.storage.newyanxunmrfz_addTempSkill = true;
          }, player);
          player.when({ player: "phaseEnd" }).then(() => {
            player.removeMark("newyanxunmrfz", player.countMark("newyanxunmrfz"), false);
            game.broadcastAll((player2) => {
              delete player2.storage.newyanxunmrfz_addTempSkill;
            }, player);
          }).assign({
            mod: {
              maxHandcard(player2, num) {
                return num += player2.countMark("newyanxunmrfz");
              }
            }
          });
        }
    ...
    newfeixuemrfz: {
      ...
      async content(event, trigger, player) {
        ...
        for (let target of trigger.targets) {
          const { bool } = await target.chooseToRespond().set("prompt", `请打出一张【闪】，否则${get.translation(trigger.card)}不可响应`).set("filterCard", function(card, player2) {
            if (get.name(card) !== "shan") return false;
            return lib.filter.cardRespondable(card, player2);
          }).set("ai", (card) => {
            _status.event.getParent(4);
            let player2 = get.event().targetx;
            get.event().targetxx;
            let cardx = get.event().cardx;
            if (player2.hasSkillTag("freeShan")) return 1;
            if (
              //@ts-ignore
              player2.countCards("hs", (c) => {
                return get.canRespond(cardx, player2).includes(c.name);
              }) < cardx.name === "sha" ? 2 : 1
            )
              return -1;
          }).set("targetx", target).set("targetxx", player).set("cardx", trigger.card).forResult();
    ...
    xinyunmrfz: {
      ...
      async content(event, trigger, player) {
        ...
        target.when({
          ...
          game.broadcastAll((player2) => {
            delete player2.storage.xinyunmrfz;
            delete player2.storage.xinyunmrfz_ban;
          }, player);
        });
        while (true) {
          ...
          await target.chooseToUse().set("filterCard", function(card, player2, event2) {
            let cards2 = player2.storage.xinyunmrfz_cards;
      ...
      onuse(result, player) {
        ...
        game.broadcastAll(
          function(card2, player2) {
            player2.storage.zongyinmrfz.add(get.suit(card2));
          },
          //@ts-ignore
          card,
          player
      ...
      subSkill: {
        handLimit: {
          ...
          async content(event, trigger, player) {
            ...
            if (player.getSkills().some((skill) => {
            ...
            player.when({ global: "phaseEnd" }).then(() => {
              ...
              mod: {
                maxHandcard(player2, num) {
                  return num + player2.getStorage("zhanwangmrfz");
                }
              }
            });
          }
            ...
            async precontent(event, trigger, player2) {
              ...
              if (target.getSkills().filter((skill) => {
                ...
                target.when({ player: "phaseEnd" }).then(() => {
                  player2.unmarkSkill("chongzhoumrfz");
                }).assign({
                  mod: {
                    maxHandcard: function(player3, num) {
                      return num += 2;
                    }
                  }
                });
    ...
    zhuijimrfz: {
      ...
      async content(event, trigger, player) {
        ...
        if (obj.cards.length > 0) {
          ...
          game.broadcastAll(
            function(info, id, player2) {
              let { targets: targets2, cards } = info;
              var dialog = ui.create.dialog(`因${get.translation(player2)}的【追缉】而展示的牌`, cards);
              dialog.videoId = id;
        ...
        eff: {
          ...
          async cost(event, trigger, player) {
            ...
            if (storage.includes("damage") || storage.includes("draw")) {
              ...
              const { bool } = await player.chooseBool().set("prompt", str).set("ai", () => aiBool).forResult();
              event.result = {
                bool,
                cost_data: {
              ...
              const { index } = await player.chooseControl("选项一", "选项二", "cancel2").set("choiceList", list).set("ai", () => {
                let player2 = get.player();
                let target = get.event().target;
                return get.damageEffect(target, player2, player2) > 0 ? 1 : 0;
    ...
    youhuomrfz: {
      ...
      async cost(event, trigger, player) {
        ...
        event.result = await player.chooseCard().set("filterCard", (card, player2) => player2.canRecast(card)).set("prompt", get.prompt("youhuomrfz")).set(
          "prompt2",
      ...
      async content(event, trigger, player) {
        ...
        if (get.tag(event.cards[0], "damage") > 0) {
          ...
          player.when({
            ...
            return event2.name !== "damage" || event2.card === trigger.card;
          }).then(() => {
    ...
    pingyimrfz: {
      ...
      async cost(event, trigger, player) {
        event.result = await player.chooseTarget().set("prompt", get.prompt("pingyimrfz")).set("prompt2", get.prompt2("pingyimrfz").replace(/###([^#]+)###/g, "")).set("filterTarget", (card, player2, target) => {
      ...
      async content(event, trigger, player) {
        ...
        await player.chooseToUse(target).set("prompt", prompt).set("filterCard", (card, player2, event2) => {
        ...
        if (player.storage.pingyimrfz_check) {
          ...
          next.player = player;
          next.setContent(async function(event2, trigger2, player2) {
            await lib.skill.pingyimrfz.cost(event2, trigger2, player2);
            if (event2.result.bool !== true) return;
            event2.targets = event2.result.targets;
            await lib.skill.pingyimrfz.content(event2, trigger2, player2);
          });
        }
      },
    ...
    beinuomrfz: {
      ...
      async precontent(event, trigger, player) {
        ...
        if (get.type2(last) !== get.type2(earliest)) {
          player.when({ player: "useCardAfter" }).filter((event2, player2) => {
            return event2.cards && event2.cards[0] === cards[0];
          }).then(() => {
            player.draw(2);
          }).then(() => {
            player.chooseCard("你可以将一张手牌视为你最先获得的牌").set("filterCard", (card) => {
              ...
              if (["sha", "tao"].includes(card.name)) return 10 + player2.hp < 3 ? get.value(card) : 0;
      ...
      subSkill: {
        ...
        tip: {
          ...
          lastDo: true,
          async content(event, trigger, player) {
            for (let card of player.getCards("h").filter((card2) => card2.storage && typeof card2.storage.beinuomrfz === "number")) {
    ...
    ronghuimrfz: {
      ...
      async content(event, trigger, player) {
        ...
        player.when({ player: "phaseDiscardBegin" }).step(async (event2, trigger2, player2) => {
          const discardPlie2 = Array.from(ui.discardPile.childNodes);
          const cards2 = player2.getCards("h");
          await player2.lose(cards2);
          await player2.gain(discardPlie2, "gain2");
          player2.logSkill("ronghuimrfz");
        });
      },
      subSkill: {
        lose: {
        ...
        result: {
          player(player) {
            let transferValue = function(cards2) {
              return cards2.map((card) => player.getUseValue(card)).reduce((acc, curr) => acc + curr, 0);
            };
            let filterCards = function(cards2) {
              return cards2.filter(
                (card) => player.getCardUsable2(card) > 0 && (get.type(card) === "basic" || get.type(card) !== "basic" && player.countUsed(card) < 1)
              );
            };
