...
const legend1SJZX = {
  ...
  skill: {
        ...
        trick: {
          ...
          async content(event2, trigger2, player2) {
            ...
            if (result2.targets) {
              ...
              if (trigger2.targets.includes(result2.targets[0])) trigger2.targets.removeArray(result2.targets);
              else trigger2.targets.addArray(result2.targets);
            }
          }
        },
        gain: {
    ...
    xuegemrfz: {
      ...
      async content(event2, trigger2, player2) {
        ...
        if (result2.targets) {
          ...
          if (result2.targets[0].hp > player2.hp || player2.getExpansions("xuyingmrfz").length >= 3) return;
        }
    ...
    shixingmrfz: {
      ...
      async content(event2, trigger2, player2) {
        const expansions = player2.getExpansions("shixingmrfz");
        let prompt = (player3) => {
          const exps = player3.getExpansions("shixingmrfz");
        ...
        const { cards } = await player2.chooseCard("h").set("prompt", prompt(player2)).set("filterCard", (card) => {
          get.player();
          const cards2 = ui.selected.cards;
          if (cards2.length > 0) return get.type2(card) === get.type2(cards2[0]);
          return true;
        }).set("selectCard", [1, 2]).set("ai", (card) => {
          const player3 = get.player(), exps = player3.getExpansions("shixingmrfz");
          let val = 0;
          ...
          if (player3.hp < 2 && exps.some((card2) => get.type2(card2) === "trick")) val += 2;
          if (get.type2(card) === "trick") {
            if (player3.skipList.includes("phaseUse")) {
              val -= 10;
            } else val += 4;
          }
          return val;
        ...
        if (expansions.length < 1) {
          player2.addToExpansion(cards, player2, "giveAuto").gaintag.add("shixingmrfz");
        } else {
          if (expansions.length === 1 && cards.length === 1 && get.type2(cards[0]) === get.type2(expansions[0])) {
            player2.addToExpansion(cards, player2, "giveAuto").gaintag.add("shixingmrfz");
          } else {
            await player2.discard(expansions);
            player2.addToExpansion(cards, player2, "giveAuto").gaintag.add("shixingmrfz");
          }
        }
      },
      ...
      subSkill: {
        ...
        lose: {
          ...
          filter(event2, player2) {
            const tags = event2.gaintag_map;
            return tags && event2.cards.some((card) => {
              const id = card.cardid || "";
              return Object.keys(tags).includes(id) && tags[id].includes("shixingmrfz");
            });
          },
          async content(event2, trigger2, player2) {
            const tags = trigger2.gaintag_map;
            if (!tags) return;
            const cards = trigger2.cards.filter((card) => {
              const id = card.cardid || "";
              return Object.keys(tags).includes(id) && tags[id].includes("shixingmrfz");
            });
            if (!cards) return;
            ...
            switch (type) {
              ...
              case "equip": {
                if (game.hasPlayer((char) => char !== player2 && char.countCards("he") > 0)) {
                  const { targets } = await player2.chooseTarget().set("prompt", "是否发动【诗形】？").set("prompt2", "你可以弃置一名其他角色一张牌").set("filterTarget", (card, player3, target) => {
                    return target !== player3 && target.countCards("he") > 0;
                  }).set("ai", (target) => get.attitude2(target) < 0).forResult();
                  if (!targets) break;
                  player2.discardPlayerCard(targets[0], true, "he").set("prompt", "【诗形】:请选择你要弃置的牌");
                }
              }
            }
          }
    ...
    nianshoumrfz: {
      ...
      subSkill: {
        ...
        usesha: {
          ...
          trigger: { player: "useCardToPlayered" },
          filter: function(event2, player2) {
            let targetx = event2.targets, num = 0;
            if (targetx.length < 1 || event2.card.name != "sha" || player2.getExpansions("nianshoumrfz").length == 0) return false;
          ...
          async cost(event2, trigger2, player2) {
            if (trigger2.targets.length === 1) {
              ...
              event2.result = {
                bool,
                cost_data: {
                  target: trigger2.targets[0]
                }
              };
            } else {
              const result2 = await player2.chooseTarget(true, (card, player3, target) => {
          ...
          async content(event2, trigger2, player2) {
            const target = event2.cost_data.target;
            const { links } = await player2.chooseButton(["选择一个'巨剑'", player2.getExpansions("nianshoumrfz")], true).forResult();
        ...
        eff2: {
          ...
          filter: function(event2, player2) {
            ...
            return event2.type === "loseToExpansion" && event2.cards.some((card) => {
              const id = card.cardid || "";
          ...
          async content(event2, trigger2, player2) {
            ...
            for (const card of cards) {
              ...
              if (lib.skill[name] && player2.hasSkill(name)) {
                player2.removeSkill(name);
              }
            }
          }
        },
