  ...
  skill: {
    ...
    pozemrfz: {
      ...
      async content(event, trigger, player) {
        ...
        const { index } = await player.chooseControl(skills.map((skill) => get.translation(skill))).set(
          "choiceList",
    ...
    wenchoumrfz: {
      ...
      onremove: true,
      async cost(event, trigger, player) {
        event.result = await player.chooseCard("he").set("prompt", get.prompt("wenchoumrfz")).set("prompt2", whichWayUtil.colorize(`你可以重铸一张牌,若你重铸的牌是：<br>①【杀】或武器牌：令包含你在内的至多两名角色将手牌调整至#r${player.storage.wenchoumrfz || 4}#并跳过此阶段；<br>②非伤害类基本牌：你视为使用一张无距离和次数限制的【杀】，然后此技能本回合失效，并令此技能中的红色数字-1。`)).set("ai", (card) => {
        ...
        if (get.subtype(card) === "equip1" || get.name(card) === "sha") {
          let adjust = function(target2) {
            ...
            if (target2.countCards("h") < num) target2.drawTo(num);
              ...
              target2.chooseToDiscard(target2.countCards("h") - num, true).set("ai", (card2) => -get.value(card2)).set("prompt", `【问仇】：将手牌调整至${num}张`).set("prompt2", whichWayUtil.colorize(`#s化干戈为玉帛，也是需要代价的#`));
            }
          };
          ...
          const {
            //@ts-ignore
            targets: [target]
          } = await player.chooseTarget(true, [0, 1]).set("prompt", whichWayUtil.colorize(`【问丑】：请选择一名其他角色，令其和#r你#将手牌调整至${player.storage.wenchoumrfz || 4}`)).set("prompt2", whichWayUtil.colorize(`#s或许，旧时的恩怨已经结束了#`)).set("filterTarget", lib.filter.notMe).set(
            "ai",
            /**@param {Player} target */
            (target2) => {
              const player2 = get.player();
    ...
    tiankonghe_zhenlimrfz: {
      ...
      async content(event, trigger, player) {
        ...
        await player.chooseUseTarget({ name: "sha", isCard: true, storage: { tiankonghe_zhenlimrfz: random } }).set("forced", true).set("addCount", false).set("nodistance", true).set("filterTarget", (card, player2, target) => targets.includes(target)).set("prompt", `【真理】:对参与拼点的其他角色视为使用一张【杀】`);
    ...
    zhiyimrfz: {
      ...
      async content(event, trigger, player) {
        ...
        if (!lib.card[`zhiyimrfz_card_${skill}`]) {
          game.broadcastAll(
            //@ts-ignore
            function(skill2, subtype2, manager2) {
              manager2.createCard(skill2, subtype2);
            },
            skill,
            //@ts-ignore
            subtype,
              ...
              if (info.characterManager?.isCharacter) {
                if (name) {
                  if (!lib.card[name]) {
                    game.broadcastAll(
                      //@ts-ignore
                      function(skill, subtype, manager2) {
                        manager2.createCard(skill, subtype);
                      },
                      //@ts-ignore
    ...
    huayingmrfz: {
      ...
      async content(event, trigger, player) {
        ...
        if (!lib.card[`zhiyimrfz_card_${skills.join("_")}`]) {
          game.broadcastAll(
            //@ts-ignore
            function(skill, subtype2, manager2, name, hp) {
              manager2.createCard(skill, subtype2, name, hp);
            },
            //@ts-ignore
            skills,
            subtype,
    ...
    pojianmrfz: {
      ...
      // @ts-ignore
      async cost(event, trigger, player) {
        player.getSkills(null, null, false).filter((skill) => {
          let info = get.info(skill);
        ...
        player.when({ player: "useCardAfter" }).filter((event2, player2) => {
          ...
          targets.forEach((target) => {
            ...
            target.when({
              ...
              player2.unmarkSkill("pojianmrfz");
              if (trigger2.name !== "useCard") return;
              if (numx > 0) trigger2.effectCount += numx;
            }).vars({
    ...
    renweimrfz: {
      ...
      async cost(event, trigger, player) {
        ...
        event.result = await player.chooseCard().set("prompt", get.prompt("renweimrfz")).set("prompt", `你可以交换牌堆顶和手牌中的一张牌<br>当前阶段:<font color='red'>${get.translation(phase)}</font>`).set("ai", (card) => {
          let phase2 = get.event().phaseName;
          let player2 = get.player();
          switch (phase2) {
            case "phaseDraw":
            case "phaseUse":
            ...
            default:
              return player2.hasSkill("tiandingmrfz") ? 0 : 6 - get.value(card);
          }
        }).set("phaseName", phase).forResult();
      },
      async content(event, trigger, player) {
    ...
    tiandingmrfz: {
      ...
      // @ts-ignore
      async content(event, trigger, player) {
        const { card, color } = await player.judge().set("judge", (card2) => {
          let color2 = get.color(card2);
          return color2 === "red" ? -4 : 0;
        }).set("judge2", (result) => result.bool === false).forResult();
          ...
          if (skill.startsWith("player_when")) {
            ...
            if (info.luwumrfz) {
              game.broadcastAll(
                //@ts-ignore
                (skill2, name) => {
                  lib.translate[skill2] = `${get.translation(name)[0]}↑`;
                },
                skill,
                info.luwumrfzName
              );
    ...
    penhuimrfz: {
      ...
      transferRGB(card) {
        let [r, g, b] = [0, 0, 0];
        let number = get.number(card), color = get.color(card), type = get.type2(card);
        let num = (number || 0) * 10;
        if (color === "red") {
          if (type === "basic") r += num;
          if (type === "equip") b += num;
      ...
      intro: {
        mark(dialog, storage) {
          ...
          function hsvToRgb(h, s, v2) {
            let f = (n, k = (n + h / 60) % 6) => v2 - v2 * s * Math.max(Math.min(k, 4 - k, 1), 0);
            return {
      ...
      async content(event, trigger, player) {
        ...
        game.broadcastAll(
          //@ts-ignore
          async function(rgb2, player2) {
            player2.storage.penhuimrfz = await lib.skill.penhuimrfz.addColors(player2.storage.penhuimrfz, rgb2);
          },
          rgb,
          //@ts-ignore
          player
        ...
        switch (color) {
          ...
          case "green":
            ...
            player.when({ player: "useCardEnd" }).then(() => {
            ...
            break;
          case "red": {
            const { targets } = await player.chooseTarget(true).set("prompt", `对一名其他角色造成一点伤害`).set("ai", (target) => get.attitude(player, target) < 0).set("filterTarget", lib.filter.notMe).forResult();
            if (!targets) return;
    ...
    cangshimrfz: {
      ...
      // @ts-ignore
      async content(event, trigger, player) {
        const { control } = await player.chooseControl(lib.suit).set("prompt", `【藏石】：请选择一种花色，当你获得这种花色的牌后你摸两张牌`).set("ai", () => {
          return "diamond";
        }).forResult();
        if (control) {
    ...
    sanyanmrfz: {
      ...
      async content(event, trigger, player) {
        ...
        const { targets } = await player.chooseTarget().set("filterTarget", lib.filter.notMe).set("selectTarget", [1, num]).set("ai", (target) => get.attitude2(target) > 0).set("prompt", `【三焰】：你可以令至多${get.cnNumber(num)}名其他角色摸一张牌`).forResult();
        if (targets) {
    ...
    dongqumrfz: {
      ...
      // @ts-ignore
      async cost(event, trigger, player) {
        event.result = await player.chooseTarget().set("prompt", get.prompt("dongqumrfz")).set("prompt2", `你可以令至多${get.cnNumber(player.maxHp)}名角色重铸一张手牌，若重铸的牌点数之和大于21，则这些角色回复一点体力`).set("filterTarget", (card, player2, target) => target.countCards("h", (card2) => target.canRecast(card2)) > 0).set("selectTarget", [1, player.maxHp]).set("ai", (target) => get.attitude2(target) > 0).forResult();
      },
      // @ts-ignore
      async content(event, trigger, player) {
        ...
        while (targets.length > 0) {
          ...
          if (target) {
            let { cards: cardsx } = await target.chooseCard(true).set("ai", (card) => -get.value(card)).set("prompt", "【冬去】:请重铸一张手牌").set("filterCard", (card) => get.event().targetx.canRecast(card)).set("targetx", target).forResult();
            if (cardsx) {
              cards.push(...cardsx);
    ...
    qixiemrfz: {
      ...
      async content(event, trigger, player) {
        ...
        const { control } = await player.chooseControl(subtypes).set("prompt", "请选择一个装备栏").set("ai", () => {
          let subtypes2 = get.event().subtypes;
          let player2 = get.player();
          let list = subtypes2.filter((i) => !player2.getEquip(i));
          return list.length > 0 ? list.randomGet() : subtypes2.randomGet();
        }).set("subtypes", subtypes).forResult();
      ...
      async createEquip(skill, subtype) {
        if (!lib.card["qixiemrfz_" + skill]) {
          ...
          var card = {
            ...
            enable: true,
            selectTarget: -1,
            filterCard(card2, player, target) {
