...
import ConfOverride from "./confOverride.vue.js";
import { confictedSkin } from "./confictedSkin.js";
class WhichWaySkin {
  /**
   * 皮肤组件初始化
   */
  async init() {
    await whichWayUtil.measureExecutionTime(async () => {
        ...
        if (mode === "sjzx") {
          whichWayAPIOverride.appendHook("lib.element.player.init", {
            before(character, character2) {
              ...
              if ((function() {
                let mode2 = lib.config.mode;
                switch (mode2) {
                  case "single":
                    return get.config("double_character", mode2) !== "single";
                  default:
                    return get.config("double_character", mode2);
                }
              })()) {
                whichWayToast.showToast("【驶舰之向】:双将时无法使用动皮！", 5e3, "bottomCenter", "sjzx_doubleCharacterWarn");
        ...
        options: {
          ...
          async onclick() {
            if (window.whichWaySave.updatingSkinData) {
              whichWayToast.showToast("[驶舰之向] 正在更新皮肤数据，请勿重复操作...", 3e3, "topLeft", "configTips_updateSkinData_updatingSkinData");
              return;
            }
            ...
            await whichWaySkin.autoUpdateSkinData(true).then(() => {
              ...
              whichWayToast.showToast("[驶舰之向] 皮肤数据更新完成!", 3e3, "topLeft", "configTips_updateSkinData");
            });
          }
        }
      }
  ...
  getCurrentSkinPath(name) {
    ...
    if (!skin) {
      if (window.whichWaySave.allCharacters.includes(name)) skin = whichWayFile.compilePath(`img:character/${name}.jpg`);
        ...
        if (char.img !== void 0) return char.img;
  ...
  showConflictedSkin() {
    ...
    createApp(ConfOverride).mount(overlay);
  ...
  syncAllSkinConfig() {
    ...
    const self = this;
    const loadFrom = (obj, isTransformed) => {
      for (const key in obj) {
        ...
        if (isTransformed) {
          ...
          if (Array.isArray(val) && val.length >= 1 && typeof val[0] === "string") {
            ...
            self._skinStore[key] = [val, void 0];
          }
        }
      }
    };
    ...
    try {
      ...
      whichWayUtil.saveConfig("skin", full, void 0);
      whichWayUtil.saveConfig("qhly_skinset", lib.config.qhly_skinset, void 0);
      whichWayUtil.saveConfig("skinConfig", window.whichWaySave.skinConfig);
    } finally {
...
const whichWaySkin = new WhichWaySkin();
