...
- 💬 **提示条（`src/tips/`）**：卡牌 / 角色上的 `.promptSJZX` 提示（原 `css/extension.css` 里那几条）已组件化为 `src/tips/promptSJZX.vue`，`.promptSJZX-Wrapper` 由组件渲染、样式是 scoped（优先级高于引擎的卡片样式，也不再依赖 `extension.css` 的加载顺序）。调用方**不要手写 DOM**：用 `whichWayTips.addPrompt(el, str, id?, del?)` / `card.addPromptSJZX(str, id?)`，或 `src/tips/prompt.ts` 的 `addPromptTo(el, { id, text, type })`（同一个宿主元素共用容器，删空自动销毁）。要调外观请改那个组件，别在 `css/extension.css` 里重新定义 `.promptSJZX*`。
